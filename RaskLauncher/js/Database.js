...
var init = function (callback) {
    try {
        db = LocalStorage.openDatabaseSync(Qt.application.name, "1.0",
                                           "Database", 1000000)
    } catch (err_db) {
...
var add = function (table, data) {
    ...
    for (var k in data) {
        ...
        insert_data.values.push(
                    (typeof data[k] === 'number' ? data[k] : "'" + data[k] + "'"))
    }

    db.transaction(function (tx) {
        try {
            tx.executeSql("INSERT INTO " + table + " (" + insert_data.keys.join(
                              ', ') + ") VALUES(" + insert_data.values.join(
                              ', ') + ")")
        } catch (err) {
...
var getOne = function (table, where) {
    if (!where)
        where = "id > 0"
    var row = []
    try {
        db.transaction(function (tx) {
            var get = tx.executeSql(
...
var get = function (table, where, order) {
    if (!where)
        where = "id > 0"
    var content = []
    try {
        db.transaction(function (tx) {
            ...
            for (var i = 0; i < get.rows.length; ++i)
...
var getJoin = function (table, fields, joins, reference, where, order) {
    var content = []
    try {
        db.transaction(function (tx) {
...
var update = function (table, where, data) {
    var data_update = [], query
    for (var key in data)
        data_update.push(
                    key + "=" + (typeof data[key]
                                 === 'number' ? data[key] : "'" + data[key] + "'") + "")
    query = "UPDATE " + table + " SET " + data_update.join(
                ', ') + " WHERE " + where
